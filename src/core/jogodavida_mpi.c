...
void trocar_bordas(int *tabul, MPIInfo *info, int tam) {
    ...
    // Enviar para processo seguinte, receber do seguinte
    ...
    // Enviar para processo anterior, receber do anterior
    if (info->rank > 0) {
...
void UmaVida_MPI(int *tabulIn, int *tabulOut, int tam, MPIInfo *info) {
    int i, j, vizviv;
    
    // Processar apenas linhas locais (1 a local_rows)
    for (i = 1; i <= info->local_rows; i++) {
        for (j = 1; j <= tam; j++) {
            // Contar vizinhos (indexação local)
...
void InitTabul_MPI(int *tabulIn, int *tabulOut, int tam, MPIInfo *info) {
    ...
    // Inicializar padrão glider APENAS no processo que contém as linhas 1, 2, 3
    if (info->start_row <= 3 && info->end_row >= 1) {
        // Converter coordenadas globais para locais
        
        // Posição global (1,2) -> linha global 1
        if (1 >= info->start_row && 1 <= info->end_row) {
            int local_i = 1 - info->start_row + 1; // +1 por causa da ghost row
            tabulIn[local_i * (tam + 2) + 2] = 1;
        }
        
        // Posição global (2,3) -> linha global 2  
        if (2 >= info->start_row && 2 <= info->end_row) {
            int local_i = 2 - info->start_row + 1;
            tabulIn[local_i * (tam + 2) + 3] = 1;
        }
        
        // Posições globais (3,1), (3,2), (3,3) -> linha global 3
...
int Correto_MPI(int *tabul, int tam, MPIInfo *info) {
    ...
    // Verificar se há exatamente 5 células vivas
    if (global_count != 5) {
        return 0;
    }
        
    
    // Verificar posições específicas nas linhas finais
    int local_positions_ok = 1;
    
    // Verificar se este processo tem as linhas finais (tam-2, tam-1, tam)
    if (info->end_row >= tam - 2) {
        // Posições esperadas: (tam-2,tam-1), (tam-1,tam), (tam,tam-2), (tam,tam-1), (tam,tam)
    ...
    // Todos os processos devem concordar que as posições estão OK
...
int main(int argc, char *argv[]) {
    ...
    for (pow = POWMIN; pow <= POWMAX; pow++) {
        ...
        // Alocar memória (local_rows + 2 ghost rows)
        int local_size = (info.local_rows + 2) * (tam + 2);
        ...
        for (i = 0; i < 2 * (tam - 3); i++) {
            ...
            UmaVida_MPI(tabulIn, tabulOut, tam, &info);
            
            trocar_bordas(tabulOut, &info, tam);
            UmaVida_MPI(tabulOut, tabulIn, tam, &info);
        }
        t2 = wall_time();
        ...
        if (info.rank == 0) {
            ...
            printf("tam=%d; procs=%d; tempos: init=%7.7f, comp=%7.7f, fim=%7.7f, tot=%7.7f \n", 
                   tam, info.size, t1 - t0, t2 - t1, t3 - t2, t3 - t0);
        }
