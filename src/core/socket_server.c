...
int execute_openmp_mpi_engine(gameoflife_request_t* request, gameoflife_response_t* response) {
    ...
    if (strcmp(request->engine_type, "openmp_mpi") != 0) {
        strcpy(response->status, "ERROR");
        snprintf(response->error_message, sizeof(response->error_message), "Engine '%s' não suportado. Use 'openmp_mpi' ou 'spark'", request->engine_type);
        return -1;
    }
    ...
    if (pid == 0) {
        ...
        if (access("/app/jogodavida_openmp_mpi", F_OK) == 0) {
            execl("/usr/bin/mpirun", "mpirun", "-np", "2", "--allow-run-as-root", "/app/jogodavida_openmp_mpi", "--powmin", powmin_str, "--powmax", powmax_str, NULL);
        } else if (access("binarios/jogodavida_openmp_mpi", F_OK) == 0) {
            char current_dir[256];
            getcwd(current_dir, sizeof(current_dir));
            chdir("binarios");
            execl("/usr/bin/mpirun", "mpirun", "-np", "2", "./jogodavida_openmp_mpi", "--powmin", powmin_str, "--powmax", powmax_str, NULL);
        } else {
        ...
        if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
            ...
            strcpy(response->status, "ERROR");
            snprintf(response->error_message, sizeof(response->error_message), "Engine terminou com erro (código: %d)", WEXITSTATUS(status));
        }
    }
    return 0;
}
